            ...
            <div class="ibox float-e-margins">
                <div class="ibox-content" style="border-style:none;">
                    <select class="form-control" id="select-type">
                        @if($type == 'all')
                        <option value="1" selected>Danh sách tất cả nghệ sĩ</option>
                        <option value="2">Sắp theo tăng dần số subscribe</option>
                        <option value="3">Sắp theo giảm dần số subscribe</option>
                        <option value="4">Sắp theo tăng dần số lượng video</option>
                        <option value="5">Sắp theo giảm dần số lượng video</option>
                        @elseif($type == 'sort-by-subscribe-asc')
                        <option value="1">Danh sách tất cả video</option>
                        <option value="2" selected>Sắp theo tăng dần số subscribe</option>
                        <option value="3">Sắp theo giảm dần số subscribe</option>
                        <option value="4">Sắp theo tăng dần số lượng video</option>
                        <option value="5">Sắp theo giảm dần số lượng video</option>
                        @elseif($type == 'sort-by-subscribe-desc')
                        <option value="1">Danh sách tất cả video</option>
                        <option value="2">Sắp theo tăng dần số subscribe</option>
                        <option value="3" selected>Sắp theo giảm dần số subscribe</option>
                        <option value="4">Sắp theo tăng dần số lượng video</option>
                        <option value="5">Sắp theo giảm dần số lượng video</option>
                        @elseif($type == 'sort-by-count-video-asc')
                        <option value="1">Danh sách tất cả video</option>
                        <option value="2">Sắp theo tăng dần số subscribe</option>
                        <option value="3">Sắp theo giảm dần số subscribe</option>
                        <option value="4" selected>Sắp theo tăng dần số lượng video</option>
                        <option value="5">Sắp theo giảm dần số lượng video</option>
                        @elseif($type == 'sort-by-count-video-desc')
                        <option value="1">Danh sách tất cả video</option>
                        <option value="2">Sắp theo tăng dần số subscribe</option>
                        <option value="3">Sắp theo giảm dần số subscribe</option>
                        <option value="4">Sắp theo tăng dần số lượng video</option>
                        <option value="5" selected>Sắp theo giảm dần số lượng video</option>
                        @endif
                    </select>
                    <div class="ibox float-e-margins">
                        <div class="ibox-content" style="border-style:none;">
                                ...
                                <table class="table dataTables">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width:15%;">Lượt theo dõi</th>
                                            <th style="width:15%;">Số video</th>
                                    ...
                                    @if(count($users) > 0)
                                        @foreach ($users as $user)
                                            <tr>
                                                <td>{{$i += 1}}</td>
                                                <td>
                                                    <img style="width: 4rem;border-radius: 50%;" src="{{$user->ava_src ? $user->ava_src : 'Undefined'}}" alt="{{$user->ava_src}}">
                                                    <a target="_blank" class="tr-name" href="/profile-artist/{{$user->id ? $user->id : 0}}">{{$user->name ? $user->name : "Undefined"}}</a>
                                                </td>
                                                
                                                <td style="font-weight: bold;">{{$user->count_subscribe ? $user->count_subscribe : 0}}</a></td>
                                                <td style="font-weight: bold;">{{$user->count_video ? $user->count_video : 0}}</a></td>
                                                <td>
                                                    @if($user->disable == 1)
    ...
    $(document).ready(function() {
        ...
        $('#select-type').on('change', function (e) {
            ...
            if(select ==1){
                ...
                var type = 'sort-by-subscribe-asc';
            }else if(select == 3){
                var type = 'sort-by-subscribe-desc';
            }else if(select == 4){
                var type = 'sort-by-count-video-asc';
            }else if(select == 5){
                var type = 'sort-by-count-video-desc';
            }
            window.location='/admin/analytics-artist/'+ type;
        });
    });
</script>
<script>
